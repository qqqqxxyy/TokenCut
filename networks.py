...
ROOT_PATH = load_path_file()['root_path']
...
def get_model(arch, patch_size, device):
    ...
    if "moco" in arch:
        ...
        if arch == "mae_vit_base" and patch_size == 16:
            url = os.path.join(ROOT_PATH,"weight","pretrained_weight/mae_pretrain_vit_base.pth")
        model = vits.__dict__[arch](num_classes=0)
    elif "vit" in arch:
        if arch == "vit_small" and patch_size == 16:
            url = os.path.join(ROOT_PATH,"weight","pretrained_weight/dino_deitsmall16_pretrain.pth")

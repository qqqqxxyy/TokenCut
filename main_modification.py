...
if __name__ == "__main__":
    ...
    for im_id, inp in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            if "vit"  in args.arch:
                ...
                for elem in range(11,12):
                    ...
                    #对目标图像施加几何变换
                    img,w_featmap,h_featmap = model.rot(img,w_featmap,h_featmap)
                    ...
                    if args.dinoseg:
                        ...
                        if args.which_features == "k":
                            ...
                            #PCA将维处理：
                            '''
                            cls_token = feat[0,0:1,:].cpu().numpy() 
                            feat = feat[0,1:,:]
                            feat = feat.cpu().numpy()
                            ...
                            feat = normalize(feat,norm='l2',axis=1,copy=True,return_norm=False)
                            for i in range(10):
                                k = np.expand_dims(feat[:,i],axis=1)
                                feats.append( k)
                            '''
        ...
        if not args.dinoseg:
            # feat1 = feats[0]
            # feat1 = feat1.mean(dim=2)
            # feat1 = feat1[:,1:]
            # feat1 = feat1.reshape(w_featmap,h_featmap)
            # feat1 = F.normalize(feat1, p=2)
            # ipdb.set_trace()
            eigenvector_list = []
            pred, objects, foreground, seed , bins, eigenvector= ncut(feats[0], [\
            ...
            # eigenvector_list.append(eigenvector2)
            # #计算余弦相似度
            # eigen1 = feat1
            # eigen2 = feat2
            # sim = torch.cosine_similarity(eigen1,eigen2,dim=2).sum()/feat1.shape[1]
            # print(sim)
        ...
        if args.visualization :
            ...
            for eigenvector in eigenvector_list:
                ...
                #保存图片
                vis.save_htmp(mask_pre,bbox_pre=list(pred)#,bbox_gt= list(gt_bbxs.astype('int'))
                    ,image= img,paths=[save_paths])
                    # ,image= img,paths=[save_paths])

                # mask_pre2 = torch.tensor(eigenvector2).unsqueeze(0)   
                # mask_pre2 = F.interpolate(mask_pre2.unsqueeze(0),size=(img.shape[1],img.shape[2]),mode='bilinear',align_corners=True)                
                # vis.save_htmp(mask_pre2,bbox_gt= list(gt_bbxs),bbox_pre=list(pred2)
                # ,image= img,paths=[save_paths])

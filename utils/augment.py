...
class RandomHorizontalFlipBBox2(object):
...
class RandomResizedBBoxCrop2(object):
    """Crop the given PIL Image to random size and aspect ratio.
    对PIL图像进行随机尺寸的剪裁，先剪裁在回放至原始大小
    ...
    Args:
        size: expected output size of each edge输出的图像尺寸
        scale: range of size of the origin size cropped随机剪裁尺寸
        ratio: range of aspect ratio of the origin aspect ratio cropped随机剪裁纵横比
        interpolation: Default: PIL.Image.BILINEAR
    """

    def __init__(self, size, scale=(0.2, 1.0), ratio=(3. / 4., 4. / 3.), interpolation=InterpolationMode.BILINEAR):
...
class ResizedBBoxCrop2(ResizedBBoxCrop):
    '''进行多标签的插值,主要是get_params中生成intersec的部分
    '''
    @staticmethod
    def get_params(img, bbox, size):
        #resize to 256
...
class CenterBBoxCrop2(CenterBBoxCrop):
    '''进行多标签的中心剪裁,同样,只需要改动get_param部分
    '''
    def __call__(self, img, bbox):
        """
        Args:
        ...
        crop_bbox = []
        for box in bbox:
            i, j, th, tw, cp_box = self.get_params(img, box, self.size)
            crop_bbox.append(cp_box)
        return F.center_crop(img, self.size), np.array(crop_bbox)